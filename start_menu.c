...
Rectangle pop = {.x = (SCREEN_W / 2) - (rectWidth / 2),
...
void PopWelcome(Rectangle pop) {
  ...
  Rectangle quit_button = {.x = (SCREEN_W / 2) - ((pop.width / 4) / 2),
  ...
  DrawText(up_title, pop.x + (pop.width / 2) - (up_title_width / 2), pop.y + 35,
           up_title_size, BLACK);
  DrawText(title, pop.x + (pop.width / 2) - (title_width / 2), pop.y + 60,
           title_size, VIOLET);

  const char *text_mode = "Choose the game mode";
  GuiDrawText(
      text_mode,
      (Rectangle){SCREEN_W / 2 - 60, SCREEN_H / 2 - 7, 120, (pop.height / 7)},
      1, BLACK);
  ...
  if (GuiButton((Rectangle){SCREEN_W / 2, central_buttons.y,
                            central_buttons.width, central_buttons.height},
                "Multiplayer")) {
    show_start_popup = false;
...
void PopIPSettings(Rectangle pop, IpSettings *sets, char ip_str[],
  ...
  Rectangle cancel_button = {.x = (SCREEN_W / 2) - (pop.width / 3),
                             .y = SCREEN_H - pop.height + 15,
                             .width = (pop.width / 4),
                             .height = (pop.height / 7)};

  Rectangle apply_button = {.x = (SCREEN_W / 2) + (pop.width / 3) -
                                 (pop.width / 4),
                            .y = SCREEN_H - pop.height + 15,
                            .width = (pop.width / 4),
                            .height = (pop.height / 7)};
  ...
  DrawText(up_title, pop.x + (pop.width / 2) - (up_title_width / 2), pop.y + 35,
           up_title_size, BLACK);
  DrawText(title, pop.x + (pop.width / 2) - (title_width / 2), pop.y + 63,
           title_size, BLACK);

  DrawText(ip_title, (SCREEN_W / 2) - 90, (SCREEN_H / 2) - 7, ip_port_size,
           BLACK);
  DrawText(port_title, (SCREEN_W / 2) + 75 - port_width, (SCREEN_H / 2) - 7,
           ip_port_size, BLACK);

  Rectangle ip_input_box = {.x = (SCREEN_W / 2) - 105,
                            .y = (SCREEN_H / 2) + 10,
                            .height = 30,
                            .width = 90};

  if (GuiTextBox(ip_input_box, ip_str, 16, true)) {
    sscanf(ip_str, "%hhu.%hhu.%hhu.%hhu", &sets->ip[0], &sets->ip[1],
           &sets->ip[2], &sets->ip[3]);
